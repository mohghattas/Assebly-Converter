...
my ($species,$slice, $ensembl_genomes );
...
Usage:
  ...
  $0 --help
    ...
    --slice / -sl   the slice  needed to map to
...
Example usage:
  ...
  $0 -s human -sl chromosome:NCBI36:X:1:10000:1
...
my $slice_adaptor = $registry->get_adaptor( $species, 'Core', 'Slice' );
  ...
  # delete any comment from the current line.
  $slice =~ s/\s*#.*$//;

  # Skip whitespace line if exist
  if ( $slice =~ /^\s*$/ ) { next }

  #deleting spaces from the begining and ending of the line
  $slice =~ s/^\s+|\s+$//;

  # check if the location of string is correctly formated
  # the correct string should look like chromosome:GRCh37:X:1000000:1000100:1
  ...
  if ( ($old_cs_name, $old_version, $old_sr_name, $old_start, $old_end, $old_strand) = $slice =~ $regex) {
  } else {
